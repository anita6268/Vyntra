...
function PinnedMessagesPanel({ messages, isLoading, error, onRetry }) {
  const { authUser } = useAuthStore();

  const pinned = (messages || []).filter(
    (m) => Array.isArray(m.pinnedBy) && m.pinnedBy.some((id) => String(id) === String(authUser?._id))
  );
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
        <div className="flex size-10 items-center justify-center rounded-full border border-white/10 bg-white/5">
          <PinIcon className="size-5 animate-pulse text-amber-400" />
        </div>
        <p className="text-xs text-[color:var(--text-muted)]">Loading pinned messages…</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
        <div className="flex size-10 items-center justify-center rounded-full border border-white/10 bg-white/5">
          <PinIcon className="size-5 text-rose-400" />
        </div>
        <p className="text-xs text-[color:var(--text-primary)]">Failed to load pinned messages</p>
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
        <div className="flex size-16 items-center justify-center rounded-full border border-white/10 bg-white/5">
          <PinIcon className="size-7 text-[color:var(--text-muted)]" />
        </div>
        <p className="text-sm font-medium text-[color:var(--text-primary)]">No pinned messages</p>
        <p className="max-w-xs text-xs text-[color:var(--text-muted)]">
          Hover over a message and tap 📌 in the action toolbar to pin it here.
        </p>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {pinned.map((msg, i) => {
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-center gap-1.5 text-[10px] font-medium text-[color:var(--accent-3)]">
